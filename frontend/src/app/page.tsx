...
const Home: React.FC = () => {
  ...
  return (
    ...
    //       />
    //       <select onChange={(e) => setInterval(e.target.value)} value={interval} style={{ marginRight: '10px', padding: '5px' }}>
    ...
    //       </select>
    //       <button onClick={handleForecast} style={{ padding: '5px 10px' }}>Generate</button>
    //     </div>
    ...
    //     </div>
    //   </div>
    //   <div style={{ flex: 1,  backgroundColor: '#f8f9fa', borderLeft: '1px solid #ddd', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
    //     {stockInfo ? (
    //       <div style={{ width: '100%', maxWidth: '300px', textAlign: 'left' }}>
    //         <h1 style={{ fontSize: '2rem', marginBottom: '10px' }}>{stockInfo.name}</h1>
    //         {/* <p style={{ fontSize: '1.5rem', marginBottom: '10px', color: '#555' }}>{stockInfo.description}</p> */}
    //         <p style={{ fontSize: '1rem', marginBottom: '0px' }}><strong>Website:</strong> <a href={stockInfo.website} target="_blank" rel="noopener noreferrer">{formatWebsite(stockInfo.website)}</a></p>
    //         <p style={{ fontSize: '1rem', marginBottom: '0.75rem' }}><strong>Sector:</strong> {stockInfo.sector}</p>

    //         <h2 style={{ fontSize: '1.25rem', marginBottom: '10px' }}>Financials</h2>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>Market Cap:</strong> ${formatNumber(stockInfo.financials.marketCap)}</p>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>EBITDA:</strong> ${formatNumber(stockInfo.financials.ebitda)}</p>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>P/E Ratio:</strong> {stockInfo.financials.peRatio}</p>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>Close:</strong> ${formatNumber(stockInfo.financials.close)}</p>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>Open:</strong> ${formatNumber(stockInfo.financials.open)}</p>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>High:</strong> ${formatNumber(stockInfo.financials.high)}</p>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>Low:</strong> ${formatNumber(stockInfo.financials.low)}</p>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>Volume:</strong> {formatNumber(stockInfo.financials.volume)}</p>
    //         <p style={{ fontSize: '1rem', marginBottom: '5px' }}><strong>Percent Change:</strong> {stockInfo.financials.pctChange ?? '-'}%</p>
    //       </div>
    //     ) : (
  ...
  {/* <div style={{ flex: 1, backgroundColor: '#fff', borderLeft: '1px solid #ddd', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)', borderRadius: '10px', margin: '20px', padding: '20px' }}> */}
  <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    {stockInfo ? (
      <div style={{ width: '100%', maxWidth: '350px', padding: '20px', backgroundColor: '#fff', borderLeft: '1px solid #ddd', boxShadow: '0 0 10px rgba(0, 0, 0, 0.1)', borderRadius: '10px', textAlign: 'left' }}>
        ...
        <p style={{ fontSize: '1rem', marginBottom: '5px', color: '#555' }}><strong>Percent Change:</strong> {stockInfo.financials.pctChange ?? '-'}%</p>
      </div>
    ) : (
      <p>Loading stock information...</p>
    )}
